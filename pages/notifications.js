...
const Notifications = () => {
  ...
  const handleMarkAllRead = async () => {
    try {
      if (window.confirm('คุณต้องการทำเครื่องหมายว่าอ่านแล้วทั้งหมดหรือไม่?')) {
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        {notifications.some((n) => !n.isRead) && (
          <button
            ...
            className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded hover:bg-blue-700 transition shadow"
          >
            อ่านทั้งหมด
          </button>
        )}
      ...
      {notifications.length === 0 ? (
        ...
        <ul className="space-y-4">
          {notifications.map((notification) => (
            <li
              key={notification.id}
              className={`p-4 rounded-lg shadow-md ${notification.isRead ? 'bg-gray-100' : 'bg-blue-50'
                }`}
            >
              ...
              <button
                ...
                className={`mt-2 text-sm font-medium px-4 py-2 rounded ${notification.isRead
