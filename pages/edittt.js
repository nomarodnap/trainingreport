...
const Form = () => {
  ...
  // ตรวจสอบค่า totalCost และตั้งค่าตั้งต้นเมื่อเปลี่ยนค่าโดยตรง
  useEffect(() => {
    setFormData((prevFormData) => {
      const hasCost =
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const formatDate = (date) => {
        return date ? format(date, 'yyyy-MM-dd') : null; // ใช้ date-fns หรือแปลงวันที่ให้ถูกต้อง
      };
        ...
        <div className="bg-white shadow-xl rounded-lg w-full max-w-3xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="mb-4 px-4 py-3 bg-blue-50 border border-blue-200 rounded text-gray-800 text-sm">
              <p>
                กรุณาแนบเอกสาร PDF หรือรูปภาพ (JPG, JPEG, PNG) ขนาดไม่เกิน <b>5MB</b>
              </p>
              <p>
                หากมีหลายหน้า ให้รวมเป็นไฟล์ PDF และหากต้องการความช่วยเหลือในการแปลงไฟล์
                <a
                  href="/pdf-guide.html"
                  target="_blank"
